...
// public static final int PRODUCER_BATCH = 10; // make sure MSGS %
// PRODUCER_BATCH == 0
...
// Executors.newFixedThreadPool(PRODUCERS + WORKERS +
...
// 10, 2);
// PostgresDataSourceProvider dataSource = new
// PostgresDataSourceProvider(configuration);
...
// threadPool.submit(() -> {
// while (!stop.get()) {
// List<Message> pop = Q.pollMessages(QUEUE, WORKER_BATCH,
...
//
// if (size == 0 && producers.stream().map(Future::isDone).reduce(true, (b1, b2)
// -> b1 &&
// b2)) {
...
// LOGGER.info("Throughput {} msgs/second", ((MSGS * PRODUCERS) / (elapsed *
// 1.0)) * 1000);
...
// }
// throw new RuntimeException("Popped " + poppedCoutner.get() + " != produced: "
// + MSGS *
// PRODUCERS);
// }
// }
